...
--Wordpress';
	...
	$ret['ldapSiteOneAdd']		= get_site_option('ldapSiteOneAdd',0);
...
function ldapOptionsPanelGeneral() {
	...
	$tSiteOneAddChecked = ''; $fSiteOneAddChecked = '';
	...
	if($ldapCreateBlog) $tBlogChecked = "checked='checked'";
        ...
        if($ldapSiteOneAdd) $tSiteOneAddChecked = "checked='checked'";
        else $fSiteOneAddChecked = "checked='checked'";
...
?>
		...
		<table class="form-table">
			...
			<tr valign="top">
			   ...
			   <td>
				...
				<br/>
				If "Yes", this will automatically create a WordPress account for any user
				that successfully authenticates against the LDAP server. The WordPress user
				account will be named the same as the LDAP username.
				<br/><br/>
				If "No", then a
				Site Admin must create a WordPress user account for the user to be able to
				log in.  The WordPress user account must be named the same as the LDAP
				username for LDAP authentication to function.
			   </td>
			</tr>
			<tr valign="top">
			   <th scope="row">Auto-Create WordPress Sites?</th>
			   <td>
				...
				<br/>
				If "Yes", this will automatically create a WordPress site for any user that successfully authenticates against the LDAP server. The blog will be named the same as the LDAP username.
				<br/><br/>
				If "No", then a Site Admin must create a WordPress site for the user to be able to log in.  
			   </td>
			</tr>
                        <tr valign="top">
                           <th scope="row">Auto-Add Users to WordPress Site #1?</th>
                           <td>
                                <input type='radio' name='ldapSiteOneAdd' id='addSiteOneYes' value='1' <?php echo $tSiteOneAddChecked; ?>/> <label for="addSiteOneYes">Yes</label>
                                <input type='radio' name='ldapSiteOneAdd' id='addSiteOneNo' value='0' <?php echo $fSiteOneAddChecked; ?>/> <label for="addSiteOneNo">No</label>
                                <br/>
				If "Yes", new users will automatically be added to site #1.  This is a legacy feature that is no longer required.
                                <br/><br/>
                                If "No", new users will not be added to any sites automatically.
                           </td>
                        </tr>
		...
		<p class="submit"><input type="submit" class="button button-primary" name="ldapOptionsSave" value="Save Options" /></p>
	</form>
<?php
}
...
function ldapOptionsPanelConnection() {
	...
	if($ldapEnableSSL == 1) {
		$tEnableSSL = "checked='checked'";
		$tEnableTLS = "";
	} elseif ($ldapEnableSSL == 2) {
		$tEnableTLS = "checked='checked'";
		$tEnableSSL = "";
	} else { 
		$fEnableSSL = "checked='checked'";
		$tEnableTLS = "";
	}
		...
		<table class="form-table">
			<tr valign="top">
			   ...
			   <td>
				...
				<input type="submit" class="button button-primary" name="ldapFixDisplayName" value="Fix Display Name"/></p>
			   </td>
			</tr>
			<tr valign="top">
			   ...
			   <td>
				...
				<input type="submit" class="button button-primary" name="ldapFixMeta" value="Fix Meta (Required if upgrading from WPMU 1.3)"/></p>
			   </td>
			</tr>
		</table>
	</form>
...
?>
		...
		<p class="submit"><input type="submit" class="button button-primary" name="ldapOptionsSave" value="Save Attributes" /></p>
	</form>
<?php
}
...
<!--                        <tr valign="top">
                ...
                <p class="submit"><input type="submit" class="button button-primary" name="ldapGroupsSave" value="Save Groups" /></p>
	</form>
<?php
}
...
function ldap_addmenu() {
	if (function_exists('add_submenu_page') && is_super_admin()) {
		...
		add_submenu_page('settings.php',__('LDAP Options','wpmuldap'),__('LDAP Options','wpmuldap'),'manage_network',basename(__FILE__),'ldapOptionsPanel');
	}
}
...
function ldapPasswordReset($value,$userID) {
        $ldap_login = get_user_meta($userID, 'ldap_login',true);
...
function wpmuLdapDisableLdapPassword($isShowingPwdFields) {
	global $userdata, $user_id, $current_user;
	$ldap_login = get_user_meta($user_id,'ldap_login',true);
	if ($ldap_login == 'true') {
		return false;
	} else {
...
function wpmuLdapFixDisplayName() {
	...
	$users = $wpdb->get_results("SELECT ID from $wpdb->users WHERE ID > 1");
	foreach ($users as $user) {
	        $ldap = get_user_meta( $user->ID, 'ldap_login', true);
		if ($ldap) {
...
<tr>
	...
	<?php } else {
		if ($user_id == 1)
			...
			$msg = "Only network admin's can update the account type.";
		if ($ldap_login == 'true') {
